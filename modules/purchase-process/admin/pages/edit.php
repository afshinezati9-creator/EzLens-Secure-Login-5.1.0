...
.ezp-editor-tool.is-ok {

    color: #86efac !important;

    background:
        rgba(134, 239, 172, .12) !important;
}
...
(function($) {
    ...
    var copyBtn        = $('#ezp-copy-btn');
    var clearBtn       = $('#ezp-clear-btn');
    ...
    function copyAllCode() {
        ...
        var codeContent = getEditorValue();
        ...
        function onCopied() {
            ...
            copyBtn.addClass('is-ok');

            showToast(
                'کل کد در کلیپ‌بورد کپی شد.',
                'success'
            );

            setTimeout(
                function() {
                    copyBtn.removeClass('is-ok');
                },
                1200
            );
        }
        ...
        function onFail() {
            ...
            try {
                ...
                if (ok) {
                    ...
                    showToast(
                        'کپی انجام نشد. دستی انتخاب کنید.',
                        'error'
                    );
    ...
    function clearAllCode() {
        ...
        var current = getEditorValue();

        if (!$.trim(current)) {

            showToast(
                'ویرایشگر از قبل خالی است.',
                'error'
            );
        ...
        if (
            !window.confirm(
                'آیا از حذف کل کد داخل ویرایشگر مطمئن هستید؟\nاین عمل فقط محتوای ادیتور را پاک می‌کند و تا ذخیره، فایل روی سرور تغییر نمی‌کند.'
            )
        ) {
            return;
        }
        ...
        if (
            ...
            editor.codemirror.setValue(emptyStarter);

            editor.codemirror.setCursor(2, 0);
            ...
            $('#ezp-code').val(emptyStarter);

        }

        showToast(
            'کل کد حذف شد. برای اعمال روی فایل، ذخیره کنید.',
            'success'
        );
